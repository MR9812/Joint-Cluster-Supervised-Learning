...
def train(model, train_loader, xs, y_true, optimizer, cluster_num_init, num_node, batch_size):
    model.train()
    ...
    for batch in train_loader:
        ...
        node_label = one_hot(y.long().reshape(1,-1).squeeze(), 2).cuda()
...
def main():
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            loss = train(model, train_loader, xs_train, y_train_true, optimizer, args.cluster, args.num_node, args.batch_size)
