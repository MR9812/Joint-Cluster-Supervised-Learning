...
def train(model, train_loader, xs, y_true, optimizer):
    model.train()
    
    for batch in train_loader:
        optimizer.zero_grad()
        out = model([x[batch] for x in xs])
        loss = torch.nn.BCEWithLogitsLoss()(out, y_true[batch].to(torch.float))
        loss.backward()
        optimizer.step()
...
@torch.no_grad()
def test(model, xs, y_true):
    model.eval()

    y_preds = []
    loader = DataLoader(range(y_true.size(0)), batch_size=400000)
    for perm in loader:
        y_pred = model([x[perm] for x in xs])
...
def main():
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            loss = train(model, train_loader, xs_train, y_train_true, optimizer)
            #train_acc = test(model, xs_train, y_train_true)
            valid_acc = test(model, xs_valid, y_valid_true)
            ...
            if valid_acc > best_val:
                best_val = valid_acc
                final_test = test(model, xs_test, y_test_true)
