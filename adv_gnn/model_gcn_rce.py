...
class GCN(nn.Module):
    ...
    def __init__(self, nfeat, nhid, nclass, cluster, dropout=0.5, lr=0.01, weight_decay=5e-4,
    ...
    def fit(self, features, adj, labels, index, idx_train, idx_val=None, train_iters=200, initialize=True, verbose=False, normalize=True, patience=500, **kwargs):
        ...
        if idx_val is None:
            ...
            if patience < train_iters:
                ...
                self._train_with_val(labels, index, idx_train, idx_val, train_iters, verbose)
    ...
    def _train_with_val(self, labels, index, idx_train, idx_val, train_iters, verbose):
        if verbose:
            print('=== training gcn model ===')
        optimizer = optim.Adam(self.parameters(), lr=self.lr, weight_decay=self.weight_decay)
        
        #import ipdb;ipdb.set_trace()
        cluster_train_id = self.one_hot(index[idx_train], self.cluster).cuda()
        cluster_train_label = self.one_hot(labels[idx_train], self.nclass).cuda()
        ...
        idx_train_val = torch.cat((torch.from_numpy(idx_train), torch.from_numpy(idx_val)), 0)
        cluster_val_id = self.one_hot(index[idx_train_val], self.cluster).cuda()
        cluster_val_label = self.one_hot(labels[idx_train_val], self.nclass).cuda()
        ...
        for i in range(train_iters):
            ...
            node_label = self.one_hot(labels[idx_train],  self.nclass).cuda()
            cluster_label = cluster_train_label[index[idx_train]].cuda()
            ...
            y_pre = self.forward(self.features, self.adj_norm, cluster_train_id, idx_train)
            ...
            self.eval()
            node_label = self.one_hot(labels[idx_val],  self.nclass).cuda()
            cluster_label = cluster_val_label[index[idx_val]].cuda()
            rce_label = torch.matmul(node_label.unsqueeze(2), cluster_label.unsqueeze(1))
            rce_label = rce_label.view(-1, self.nclass * self.nclass)
            y_pre = self.forward(self.features, self.adj_norm, self.one_hot(index, self.cluster).cuda(), idx_train_val, True)
            y_pre = F.softmax(y_pre, dim=1)
            y_pred = y_pre.view(-1, self.nclass, self.nclass)
            y_pred = y_pred.sum(dim=2)
            acc_val = utils.accuracy(y_pred[idx_val], labels[idx_val])
    ...
    def test(self, idx_test, idx_train, idx_val, index):
        ...
        """
        self.eval()
        y_pre = self.predict(index, idx_train, idx_val)
        y_pre = F.softmax(y_pre, dim=1)
        y_pre = y_pre.view(-1, self.nclass, self.nclass)
        y_pre = y_pre.sum(dim=2)
        # output = self.output
        #loss_test = F.nll_loss(output[idx_test], self.labels[idx_test])
        acc_test = utils.accuracy(y_pre[idx_test], self.labels[idx_test])
    ...
    def predict(self, index, idx_train, idx_val, features=None, adj=None):
        ...
        if features is None and adj is None:
            idx_train_val = torch.cat((torch.from_numpy(idx_train), torch.from_numpy(idx_val)), 0)
            return self.forward(self.features, self.adj_norm, self.one_hot(index, self.cluster).cuda(), idx_train_val, True)
        else:
            if type(adj) is not torch.Tensor:
