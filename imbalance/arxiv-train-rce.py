...
def test(model, args, data, index, train_idx, valid_idx, split_idx, evaluator):
    ...
    labels = data.y.squeeze(1)
    cluster_val_id = one_hot(index[torch.cat((train_idx, valid_idx),0)], args.cluster).to(args.device)
    cluster_val_label = one_hot(labels[torch.cat((train_idx, valid_idx),0)], args.num_classes).to(args.device)
    ...
    y_pre = model(data.x, data.adj_t, one_hot(index, args.cluster).to(args.device), torch.cat((train_idx, valid_idx),0), True)
...
def main():
    ...
    #cluster_train_label = one_hot(data.y.squeeze()[data1.train_mask], args.num_classes).to(args.device)
    #cluster_train_label = torch.mm(cluster_train_id.t(), cluster_train_label)
    #cluster_train_label = cluster_train_label / cluster_train_label.sum(1).unsqueeze(1)
    #import ipdb;ipdb.set_trace()
    ...
    test_res = test_res.to(args.device)
