...
def train(model, optimizer, epoch, features, adj, idx_train, idx_val, idx_test, labels, index, num_classes, args, cluster_train_id, cluster_train_label, cluster_val_id, cluster_val_label):
    ...
    if args.metric == 'acc':
        ...
        pred = p_label.argmax(dim=-1)
        acc_val = f1_score(labels[idx_val].detach().cpu().numpy(), pred[idx_val].detach().cpu().numpy(), average='macro')
        acc_test = f1_score(labels[idx_test].detach().cpu().numpy(), pred[idx_test].detach().cpu().numpy(), average='macro')
    else:
        pred = p_label.argmax(dim=-1)
        acc_val = f1_score(labels[idx_val].detach().cpu().numpy(), pred[idx_val].detach().cpu().numpy(), average='weighted')
        acc_test = f1_score(labels[idx_test].detach().cpu().numpy(), pred[idx_test].detach().cpu().numpy(), average='weighted')
...
parser.add_argument('--dataset', type=str, default='LastFMAsia', help='Cora/CiteSeer/PubMed/DBLP/Facebook/LastFMAsia')
