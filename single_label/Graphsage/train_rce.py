...
def train(model, optimizer, data, index, args):
    ...
    cluster_train_id = one_hot(index[train_idx], args.cluster).to(args.device)
    cluster_train_label = one_hot(labels[train_idx], args.num_classes).to(args.device)
    ...
    p_label1 = one_hot(labels[train_idx],  args.num_classes).to(args.device)
...
def test(model, data, index, args):
    model.eval()
    labels = data.y
    cluster_val_id = one_hot(index[data.train_mask+data.val_mask], args.cluster).to(args.device)
    cluster_val_label = one_hot(labels[data.train_mask+data.val_mask], args.num_classes).to(args.device)
    ...
    y_pre = model(data.x, data.adj_t, one_hot(index, args.cluster).to(args.device), data.train_mask+data.val_mask, True)
    ...
    #relation ce : ground truth
    p_label1 = one_hot(labels[data.val_mask],  args.num_classes).to(args.device)
...
for run in range(args.runs):
    if args.dataset != 'Cora' and args.dataset != 'CiteSeer' and args.dataset != 'PubMed':
        ...
        data.train_mask = torch.Tensor(train_mask).bool().to(device)
        data.val_mask = torch.Tensor(val_mask).bool().to(device)
        data.test_mask = torch.Tensor(test_mask).bool().to(device)
